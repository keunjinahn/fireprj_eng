...
@app.route('/eng/make-data/event-list', methods=['GET'])
def write_event_list_api():
    file_path = '../doc/이벤트목록_영문용_20240702.xlsx'
...
@app.route('/eng/make-data/event-list-eng', methods=['GET'])
def write_event_list_eng_api():
    file_path = '../doc/이벤트목록_영문용_20240702.xlsx'
    wb = openpyxl.load_workbook(file_path)
    sheet = wb.get_sheet_by_name('Sheet')
    ...
    for i, row in enumerate(sheet.rows):
        if i < 1 : continue
        print("idx : ", i,",row : ",row[6].value)
        obj = {
            "event_desc":row[6].value,
            "event_category":row[7].value,
        }
        EventTbl.query.filter_by(id=i).update(obj)
    db.session.commit()
    print(len(data))

    return make_response(jsonify(''), 200)

@app.route('/eng/make-data/generating', methods=['POST'])
def generating_api():
...
@app.route('/eng/api/v1/login', methods=['POST'])
def login_api():
...
@app.route("/eng/api/v1/logout", methods=["POST"])
def logout_api():
...
@app.route('/eng/api/v1/save_excel/<filename>', methods=['GET'])
def save_excel_api(filename):
    upload_path = os.getcwd() + '/download'
    return send_from_directory(upload_path, filename)

@app.route('/eng/api/v1/sensor-log-chart', methods=['GET'])
...
@app.route('/eng/api/v1/insert_test_sensor_data', methods=['POST'])
...
@app.route('/eng/api/v1/sensor_data_range', methods=['POST'])
...
@app.route('/eng/api/v1/check_passwd', methods=['POST'])
