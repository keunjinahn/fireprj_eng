    ...
    <template v-slot>
      <div class="main-panel">
        <v-toolbar color="light-blue darken-4" dark flat>
          <v-toolbar-title>Repeater status</v-toolbar-title>
        </v-toolbar>
        ...
        <v-toolbar color="light-blue darken-1" dark flat>
          <v-toolbar-title>Repeater event detail</v-toolbar-title>
        </v-toolbar>         
        <v-data-table
  ...
  methods: {
    ...
    async downloadExcel() {
      ...
      .then(response => {
        ...
        link.href = url
        var download_file_name = "RepeaterStatus_" + Date.now().toString() + ".xlsx"
    ...
    return {
      repeater: {
        ...
        headers: [
          ...
          {text: "Event time", value: "last_event_time", sortable: false,align: 'center', width: 80}, 
