    ...
    <template v-slot>
      <div class="main-panel">
        <v-toolbar color="light-blue darken-4" dark flat>
          <v-toolbar-title>Data analysis</v-toolbar-title>
        </v-toolbar>
        ...
        <template v-if="chart.show">
          <v-toolbar color="light-blue darken-1" dark flat>
            <v-toolbar-title>Sensor analog value</v-toolbar-title>
          </v-toolbar>        
          ...
          <v-card height="300">
            <v-card-title class="body-1 font-weight-bold py-2">Sensor No. : {{(chart.sensor != undefined)? String(chart.sensor).padStart(3,'0') : ''}}</v-card-title>
            <v-divider/>
            <highcharts
...
export default {
  ...
  computed:{
    chartOption_sensor_logs() {
      ...
      obj.yAxis[0].title.text = "Data value";
      obj.yAxis[0].max = 10
      obj.series[0].name = "Sensor data"
      return obj
    }
  },
  data() {
    return {
      sensor: {
        ...
        headers: [
          ...
          {text: "Receiver type", value: "receiver_type",align: 'center', sortable: false, width: 20},
