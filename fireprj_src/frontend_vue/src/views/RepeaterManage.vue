    ...
    <template v-slot>
      <div class="main-panel">
        <v-toolbar color="light-blue darken-4" dark flat>
          <v-toolbar-title>Repeater management</v-toolbar-title>
        </v-toolbar>
...
export default {
  ...
  methods: {
    ...
    getPopupTitle(type){
      if(type == 'ADD') return 'Repeater register'
      if(type == 'MODIFY') return 'Repeater modify'
      if(type == 'VIEW') return 'Repeater infomation'
    },
    clearPopup(){
      this.addPopup.form = {
          repeater_idx: '',
    ...
    async downloadExcel() {
      ...
      .then(response => {
        ...
        link.href = url
        var download_file_name = "RepeaterList_" + Date.now().toString() + ".xlsx"
    ...
    return {
      ...
      repeater: {
        headers: [
          ...
          {text: 'Repeater ID', value: 'repeater_idx', sortable: false,align: 'center', width: 80},
